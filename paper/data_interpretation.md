...
**What actually happened:** 71 unique contributors from competing organizations (MetaMask, Ethereum Foundation, independent developers, academic researchers) reached binding protocol consensus in **169 days** without contracts, authority structures, or equity alignment.
...
| Total discussion records | 149 | 5,700+ |
| Unique contributors | 71 | 600+ |
| Formal voting | None | TSC git-vote (2 PRs) |

**Interpretation:**
- ERC-8004's 169-day cycle has a clear *terminus*: EIP mainnet inclusion. This creates a natural deadline that structures deliberation. Google A2A has no equivalent endpoint — the protocol is perpetually "in progress." This structural difference in time horizons may explain why ERC-8004 produced more focused deliberation per record (76.6% Technical arguments) while A2A shows a broader spread of discussion types.
...
**ERC-8004:**
- 71 unique authors across forum (113 posts) and GitHub (36 records)
- Top contributor: `MarcoMetaMask` (17 records, MetaMask) — single most active voice, yet still only 11.4% of total records
- Active critics: `pcarranzav` (16 records, Independent) and `spengrah` (10 records, Independent) show the highest Modify+Oppose stance rates, evidence of genuine adversarial review
- `lightclient` (Ethereum Foundation, ERC editor) appeared only in 2 PRs with technical reviews — the "authority" participated minimally, consistent with permissionless norms

**Google A2A:**
- 600+ unique authors, but power law concentration: top 3 humans (`holtskinner`, `darrelmiller`, `Tehsmash`) account for ~16% of all records
- `holtskinner` (Google, 439 records) alone contributes ~9% of all A2A annotated records
- `darrelmiller` (Microsoft) is simultaneously: IETF HTTPAPI WG Chair, LF A2A TSC member, and highest-activity non-Google contributor — a single individual bridging the corporate governance with formal standards infrastructure
- Key finding: **the top 5 contributors all have organizational affiliations** (Google ×3, Microsoft, Cisco). Independent contributors appear only at positions 6+.

**Interpretation:** ERC-8004 participation is more evenly distributed — no single actor controls >15% of records, and the most active critic (pcarranzav, 16 records) has no institutional affiliation. A2A shows a recognizable corporate hierarchy: Google staff set direction, Microsoft provides critical bridge to formal standards bodies, Cisco and others contribute technical depth. This is hierarchy reproducing itself in open-source form.
...
### 3.2 Google A2A Argument Distribution (4,391 annotated)
...
**Key insight:** A2A's **Google 22.6%** record concentration vs. **author concentration of ~7%** (41/560 authors) reveals that Google employees are not just participants — they are *disproportionate contributors* per person. `holtskinner` alone (439 records) explains most of this discrepancy. This is how corporate governance infiltrates an "open" project: not by excluding others, but by having employees who contribute more than anyone else.

ERC-8004's MetaMask at 11.1% of records (vs. 1.5% of authors — only MarcoMetaMask) shows the same dynamic at smaller scale: one institutional actor with high intensity.

### 5.2 Openness Index

| Case | Openness Index | Calculation |
|------|:--------------:|------------|
| ERC-8004 | **1.000** | 0 EF-labeled contributors / 71 total contributors |
| Google A2A | **0.956** | (596 − 26 Google-labeled) / 596 contributors |

*Note:* The ERC-8004 index = 1.0 because the LLM did not classify EF as the initiating institution. The actual proposer is `davidecrapis.eth` (Olas/Valory protocol), and EF's editorial involvement was classified as Independent. This is methodologically important to note: the index measures *absence of institutional capture in annotated attribution*, not formal affiliation.
...
**Data support:** MetaMask's 11.1% record share vs. 1.5% author share shows high institutional intensity but not monopoly. The presence of substantive critical voices from unaffiliated contributors (pcarranzav's Oppose rate of ~19%) who were not dismissed or silenced demonstrates that the permissionless property is operationally real, not merely formal.
...
1. **Annotation reliability:** LLM annotation with MiniMax-M2.5 achieves ~92% completion rate; no human intercoder reliability has been computed. The institution attribution for ~617/626 authors relies on LLM inference from text alone, with known errors (e.g., Tehsmash classified as Independent, actually Cisco).

2. **Comparison asymmetry:** ERC-8004 has 149 records; A2A has 5,700+. The structural difference in data volume is itself a governance finding (different transparency norms) but makes statistical comparison difficult without normalization.
