...
% Date:   2026-03-13
% Status: Working draft — all statistics from annotated_records.json (N=4,863
%         core, plus 49 git-vote), structural_metrics.csv, core_contributors.csv
%
...
governed by a corporate-led technical steering committee). Analysis of 5,749
archival governance records reveals that the DAO process achieved ratified
consensus in 169 days with 71 contributors across competing organizations, while
the corporate process remains in ongoing iteration despite 600+ participants and
...
systematically in governance mode. Analysis of 5,749 archival governance
...
\textit{ERC-8004 (Trustless Agents)} was proposed on August 13, 2025 by
\texttt{davidecrapis.eth} (Olas/Valory protocol). The standard defines a
registry for trustless AI agents on the Ethereum Virtual Machine (EVM),
enabling accountability and credentialing without centralized authority.
It was processed through the standard Ethereum Improvement Proposal (EIP)
...
as an open-source repository with 600+ contributors, governance authority
...
Total records before deduplication: 6,172. After filtering records with
fewer than 20 characters of text (bot messages, CI notifications): 5,749.
...
labels were upgraded post-hoc for 9 accounts using GitHub company fields
and Discourse bio data fetched via public APIs, following a priority cascade:
GitHub company field $>$ GitHub bio $>$ forum bio $>$ LLM inference.
...
sample. The institution field relies on LLM inference for approximately
617 of 626 unique authors; known errors include Tehsmash (Cisco, labeled
Independent). [AUTHOR: compute Cohen's $\kappa$ on a random subsample of
$N=50$ and report before submission.]
...
Discussion records & 149 & 5,600+ \\
Unique contributors & 71 & 600+ \\
Openness index & \textbf{1.000} & 0.956 \\
...
10 Oppose records in ERC-8004 originated entirely from independent
contributors---primarily \texttt{pcarranzav} (16 records, unaffiliated)
and \texttt{spengrah} (10 records, unaffiliated)---and their objections
forced specification revisions across multiple PR iterations. This
demonstrates that technical credibility alone confers influence in
...
\midrule
Independent        & \textbf{79.9} & 64.5 \\
Google             & 0 & \textbf{22.6} \\
MetaMask           & 11.1 & 0 \\
Unknown            & 6.3 & 12.6 \\
Ethereum Foundation & 2.1 & 0 \\
Microsoft          & 0 & 0.3 \\
\bottomrule
\end{tabular}
\end{table}

Both cases have high openness indices (ERC-8004: 1.000; A2A: 0.956),
suggesting that participation breadth is not the differentiating dimension.
What differs is the \textit{distribution of influence}. Google's 22.6\%
record share derives from 7.3\% of unique authors, revealing disproportionate
contribution intensity by institutional actors---particularly \texttt{holtskinner}
(Google, 439 records, $\approx 9\%$ of all A2A records). This is how corporate
governance reproduces hierarchy within an ostensibly open project: not by
excluding others, but by having employees who contribute more than everyone
else combined.

ERC-8004's MetaMask at 11.1\% of records (from \texttt{MarcoMetaMask},
the single most active contributor at 11.4\% of records) shows the same
dynamic at smaller scale. However, MetaMask's dominance was episodic and
reversed when independent contributors raised substantive objections---a
structural difference from A2A, where Google's directional control
appears persistent.
...
EIP was revised in response to concerns from an unaffiliated independent
contributor with no organizational authority, demonstrating that technical
credibility alone confers influence. Under corporate hierarchy (A2A), the
equivalent mechanism is employment reputation, which carries career risk
that suppresses opposition from non-employees.
...
The evidence: MetaMask's 11.1\% record share vs.\ 1.5\% author share
shows high institutional intensity but not monopoly, and institutional
dominance did not suppress the independent Oppose voices that forced
specification revisions. Google A2A's TSC gate is the structural opposite:
binding authority is limited to a pre-registered committee, new entrants
cannot credibly threaten to fork with equivalent legitimacy, and
institutional actors maintain persistent directional control.
...
A2A reveals that corporate openness (high openness index of 0.956) is
compatible with informal hierarchy: institutional actors can dominate
deliberation without formal authority~\cite{simcoe2012}. Openness is
...
\textbf{Comparison asymmetry:} ERC-8004 has 149 records; A2A has 5,600+.
...
been validated against a human-coded subsample. The institution field
contains known errors (Tehsmash/Cisco labeled Independent). [AUTHOR:
