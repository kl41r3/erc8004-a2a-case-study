...
**Status:** Method 1 DONE ✓ · Method 2 DONE ✓ · Method 2b DONE ✓ · Paper sections written ✓
...
| | Method 1 | Method 2 | Method 2b |
|---|---|---|---|
| **名称** | Thematic-LM | Comparative Discourse | CryptoBERT Validation |
| **来源** | ACM Web Conference 2025, DOI: 10.1145/3696410.3714595 | ACM SMSociety 2020, DOI: 10.1145/3400806.3400816 | ElKulako/cryptobert (HuggingFace) |
| **核心思路** | 4 个 LLM Agent 协作：open coding → 聚类 → 精炼 codebook → 分配主题 | BERTopic（embedding 聚类）+ Jensen-Shannon 散度跨案例对比 | 用加密领域专用 BERT 重新 embed ERC-8004，验证 Method 2 结果的鲁棒性 |
| **是否需要 LLM API** | 是（MiniMax M2.5） | 否（本地 sentence-transformers） | 否（本地 transformers） |
| **状态** | Stage 1 运行中（119/5421 已编码） | **完成** | **完成** |
| **详细文档** | [method1-thematic-lm.md](method1-thematic-lm.md) | [method2-comparative-discourse.md](method2-comparative-discourse.md) | [method2b-cryptobert-validation.md](method2b-cryptobert-validation.md) |
...
└── crypto_bert/
    └── run.py       # 入口：uv run python ...（ERC-8004 专用，无参数）
```
...
└── crypto_bert/
    ├── topics.json            # 5-topic 结果：keywords, count, pct ✓
    └── comparison_summary.md  # 与 Method 2 的收敛评估 ✓
```
