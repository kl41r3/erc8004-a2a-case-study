...
Usage:
    ...
    uv run python scripts/pipeline/run_r2.py --from-phase 3   # resume from phase 3
    uv run python scripts/pipeline/run_r2.py --skip-thematic   # skip thematic open-coding
    uv run python scripts/pipeline/run_r2.py --skip-figures    # skip figures
"""
...
from lib.paths import (DATA_ANNOTATED_R2_A2A, DATA_ANNOTATED_R2_CONSENSUS,
    DATA_ANNOTATED_R2_THEMATIC, ANALYSIS_METRICS_R2, ANALYSIS_TD_R2_CROSS_MODEL,
    ANALYSIS_ND_R2_DNA, ANALYSIS_ND_R2_SS, OUTPUT_FIGURES)
...
PHASES = [
    ...
    ], DATA_ANNOTATED_R2_A2A / "validation" / "validation_report.json"),

    (2, "Build Consensus", [
        ["uv", "run", "python", "scripts/process/build_consensus.py"],
    ], DATA_ANNOTATED_R2_CONSENSUS / "a2a_annotations.json"),

    (3, "Structural Metrics", [
        ["uv", "run", "python", "scripts/analyse/compute_metrics_r2.py"],
    ], ANALYSIS_METRICS_R2 / "structural_metrics.csv"),

    (4, "BERTopic Comparative Discourse", [
        ["uv", "run", "python", "scripts/analyse/topic_discovery/comparative_discourse/run_r2.py"],
    ], ANALYSIS_TD_R2_CROSS_MODEL / "comparative_discourse" / "divergence_table.csv"),

    (5, "A2A Thematic Open-coding (3 models)", [
        ["uv", "run", "python", "scripts/process/annotate_thematic_a2a.py", "--model", CANONICAL_MODELS[0]],
        ["uv", "run", "python", "scripts/process/annotate_thematic_a2a.py", "--model", CANONICAL_MODELS[1]],
        ["uv", "run", "python", "scripts/process/annotate_thematic_a2a.py", "--model", CANONICAL_MODELS[2]],
    ], DATA_ANNOTATED_R2_THEMATIC / "a2a" / f"{CANONICAL_MODELS[2]}_themes.json"),

    (6, "Thematic-LM Stages 2-4", [
        ["uv", "run", "python", "scripts/analyse/topic_discovery/thematic_lm/run_r2.py"],
    ], ANALYSIS_TD_R2_CROSS_MODEL / "thematic_lm" / "coded_records.json"),

    (7, "SNA Co-participation Networks", [
        ["uv", "run", "python", "scripts/analyse/build_network_r2.py"],
    ], ANALYSIS_METRICS_R2 / "network_erc_nodes.csv"),

    (8, "DNA Discourse Network Analysis", [
        ["uv", "run", "python", "scripts/analyse/network_discourse/dna/run_r2.py"],
    ], ANALYSIS_ND_R2_DNA / "dna_metrics.json"),

    (9, "Socio-semantic Bipartite Network", [
        ["uv", "run", "python", "scripts/analyse/network_discourse/sociosemantic/run_r2.py"],
    ], ANALYSIS_ND_R2_SS / "ss_metrics.json"),

    (10, "Paper Figures", [
        ["uv", "run", "python", "scripts/visualise/build_paper_figures_r2.py"],
    ], OUTPUT_FIGURES / "r2-fig-icr-heatmap.pdf"),
]
